...
- [[https://github.com/golang-standards/project-layout][Github: golang-standards/project-layout]] - Unofficial suggested layout.
- [[Insert link: https://github.com/caarlos0/env][Github: caarlos0/env]] - A simple, zero-dependencies library to parse environment variables into structs.
- [[https://github.com/samber/lo][Github: samber/lo]] - makes it easier to work with slices, maps, strings, channels, and functions.
*** Cross-compiling:
...
Or, to avoid the: =At least one output file must be specified=, error
(with =-t 0= to avoid decoding the file):

#+BEGIN_EXAMPLE shell
  ffmpeg -i <source_video> -t 0 -f null -
#+END_EXAMPLE
...
In Zsh, this can be called in a folder with this one-liner (*NOTE:*
the =.avi= extension should be changed to the file type when stripping
of the extension with =basename=:

#+BEGIN_SRC shell
  for x (*.avi(ND.)); do ffmpeg -i $x -map 0 -c:v libx265 -crf 23 -preset fast -c:a copy -c:s copy "$(basename $x .avi) - h265.mp4"; done
#+END_SRC

And an example converting Real Media files with =cook= audio codec to
h265 + mp3:

#+BEGIN_SRC shell
  for x (*.rm(ND.)); do ffmpeg -i $x -map 0 -c:v libx265 -crf 23 -preset fast -c:a libmp3lame  -c:s copy "$(basename $x .rm) - h265.mp4"; done
#+END_SRC
...
Or simpler version (with [[https://trac.ffmpeg.org/wiki/How%20to%20speed%20up%20/%20slow%20down%20a%20video#setptsfilter][double speedup]]):

#+BEGIN_SRC shell
  ffmpeg -i <source_video> -r 10 [-filter:v "setpts=0.5*PTS"] -pix_fmt rgb24 <output_gif>
#+END_SRC
*** FFmpeg trim video:
#+BEGIN_SRC shell
  # ffmpeg -ss <secs_to_start_from> <-t <duration_in_secs>|-to <secs_to_end_at>> -i <source_video> -c copy <output_video>

  # Skip first 10 seconds
  ffmpeg -i input.mp4 -ss 10 -c copy output.mp4

  # Skip first 1 minute 30 seconds
  ffmpeg -i input.mp4 -ss 00:01:30 -c copy output.mp4

  # Keep only seconds 10-60
  ffmpeg -i input.mp4 -ss 10 -to 60 -c copy output.mp4
#+END_SRC
*** [[https://trac.ffmpeg.org/wiki/Scaling][FFmpeg wiki: resize/scale by ratio for video/image]]:

Resize and keeping ratio by width or height:
#+BEGIN_EXAMPLE shell
  ffmpeg -i <input> -vf scale=<int>:-2 <scaled_output_by_width>
  ffmpeg -i <input> -vf scale=-2<int> <scaled_output_by_height>
#+END_EXAMPLE
** JSON/YAML:
*** Filetype conversions with =yq=:
...
** Mac config:
*** Intune:
**** Taming Intune when it hogs CPU:
Work laptops always use Intune for deploying policy. Often Intune
misbehaves on new OS's and spikes the CPU several 100%. Killing the
process is a temporary solution, since Intune auto-restarts and then
eventually misbehaves again.

This is a cron one-liner to kill the process when it is above a
threshold:

#+BEGIN_EXAMPLE sh
  ,* * * * 5 ps -eo pid,%cpu,comm | awk '$0 ~ /IntuneMdmDaemon/ && $2 > 190 {system("kill " $1)}'
#+END_EXAMPLE

Apply with: =sudo crontab -e=, to set it at =root= level.
*** iterm2
...
- ~Preferences > Sharing > AirPlayReceiver~ Disabled due to port conflict
  on 5000.
**** Tame Mac's Spotlight noise:
- =Settings > Spotlight= and only leave: =Applications, Conversion,
  Definition, System Settings= enabled.
- To Remove the =Developer= section from Spotlight, when you don't
  have Xcode installed:
  - =touch /Applications/Xcode.app=.
  - Reload: =Settings > Spotlight=, untick: =Developer=.
...
** Jarvis Fully Standing Desk - =Error 01= fix:
1. Check whether the desk is overloaded on one side. If it is
   overloaded, you need to remove part of the load.
2. Check whether there is mechanical abnormality, lifting
   interference.
3. If there is no mechanical abnormality on the outside, press the
   down button of the handset to enter the RST =reset= state.
   1. If the column descends synchronously, keep pressing the button
      until the reset is completed, and the handset displays numbers.
   2. if the table top is tilted, release the button immediately,
      contact after-sales.
* Docker:
...
*** Caching go dependencies between builds:
- See: [[https://docs.docker.com/reference/dockerfile/#example-cache-go-packages][Docker Docs: Cache go packages]]
  ...
  #+BEGIN_EXAMPLE dockerfile
    ...
    WORKDIR /app
    # Cache Go modules, then pull in code to avoid invalidating layer.
    # https://docs.docker.com/reference/dockerfile/#example-cache-go-packages
    COPY go.* .
    RUN --mount=type=cache,target=/root/.cache/go-build go mod download
    COPY .  .
    RUN --mount=type=cache,target=/root/.cache/go-build go build -gcflags="all=-N -l" -o app-debug ./cmd
    ...
    EXPOSE 8001 50000
    ENV PORT=8001
    ENTRYPOINT ["dlv", "exec", "--headless", "--continue", "--listen=:50000", "--accept-multiclient", "--api-version=2", "/app/app-debug"]
  #+END_EXAMPLE
*** Caching pnpm dependencies between builds:
- See: [[https://pnpm.io/docker][PNPM: Docker]]
...
- Cannot support multiple Containers re-using the same Port, due to Host-side
  conflicts.
*** list networks + ip subnet/gateway ranges:
Needed to investigate a docker network overlap issue from a script
with a hardcoded subnet. Used this for discovery:

#+BEGIN_EXAMPLE shell
  docker network ls --format '{{.Name}}' | xargs -I {} sh -c 'echo "{}"; docker network inspect {} --format "  Subnet: {{range .IPAM.Config}}{{.Subnet}} Gateway: {{.Gateway}}{{end}}"'
#+END_EXAMPLE
** Performance:
...
- Use multi-stage docker files to build code in a fat stage, but copy in the
  ...
  #+END_EXAMPLE
** How to get compressed docker image size from a registry?
- [[https://gist.github.com/MichaelSimons/fb588539dcefd9b5fdf45ba04c302db6][Github Gist: MichaelSimons - Retrieving Docker Image Sizes]].
...
- =C-u i=, Then add =R= to the =ls= command options.
- =C-u l= to redraw buffer.
- =^= to jump up to parent.
- =CM-d= to dive into a child directory.
** org-mode:
...
Or for renaming an ordered list to start from 1:
=prefix - \(\w+\) - \(.*\)\.ext → newPrefix\,(format "%02d" (+ \# 1)) - \2 (\1).ext=

#+BEGIN_EXAMPLE shell
  prefix - 001 - descriptionX.ext -> newPrefix 01 - descriptionX (001).ext
  prefix - 002 - descriptionY.ext -> newPrefix 02 - descriptionY (002).ext
  prefix - 003 - descriptionZ.ext -> newPrefix 03 - descriptionZ (003).ext
#+END_EXAMPLE
...
** Global git config to use per-VCS SSH keys for HTTPS/Git URLS:
[[*Configure git repo with explicit SSH Key:][Configure git repo with explicit SSH Key]] is great for setting unique
SSH keys per-repo, but this is how to set them on a global scale.

- *Pre-req:* Create unique SSH keys:

  #+BEGIN_EXAMPLE shell
    |- ~/.ssh/
      |- id_rsa_work_azuredevops[.pub]
...
- =~/.ssh/config=
  #+BEGIN_EXAMPLE cfg
    ...
    # Work Azure DevOps - Insecure RSA
    Host ssh.dev.azure.com
        HostName ssh.dev.azure.com
        User git
        IdentityFile ~/.ssh/id_rsa_work_azuredevops
        IdentitiesOnly yes
  #+END_EXAMPLE
- =~/.ssh/config=
  #+BEGIN_EXAMPLE cfg
    ...
    [url "git@github.com-work:<work_organisation>/"]
        insteadOf = https://github.com/<work_organisation>/
        insteadOf = git@github.com:<work_organisation>/
    [url "git@ssh.dev.azure.com:v3/"]
        insteadOf = https://dev.azure.com/
        insteadOf = git@dev.azure.com/
  #+END_EXAMPLE
- =git clone <url>= should now always work. Debug via: [[*Investigate SSH Key used in =git
  clone=:][Investigate SSH Key used in =git clone=]].
** Default Global git config:
#+BEGIN_SRC shell
  git config set  --global fetch.all true
  git config set  --global fetch.prune true
  git config set  --global fetch.pruneTags true
  git config set  --global fetch.recurseSubmodules true
  git config set  --global fetch.parallel 0
#+END_SRC
** Git Hooks:
...
#+END_EXAMPLE
** Git Notes:
Git notes are great to add information after the fact. Just remember
to do the note against the commit SHA and not a tag, else it requires
a separate =git notes show <tag>= to view the note.

#+BEGIN_EXAMPLE sh
  % git show <commit_SHA>
  commit <commit_SHA>
  Author: <git_author>
  Date:   Wed Nov 26 17:07:05 2025 +0000

      <conventional_commit>: <descriptive title>
  ...
  % git tag -a -m "Descriptive tag message" v1.2.3 <commit_SHA>
  % git show <commit_SHA>
  commit <commit_SHA> (tag: v1.2.3)
  Author: <git_author>
  Date:   Wed Nov 26 17:07:05 2025 +0000
  ...
  tag v1.2.3
  Tagger: <git_author>
  Date:   Thu Nov 27 09:54:05 2025 +0000
  ...
  Date:   Wed Nov 26 17:07:05 2025 +0000
      ...
      Ticket: <id>
  % git notes add -m "Note added after the fact, with additional detail." <commit_SHA>
  % git show <commit_SHA>
  commit <commit_SHA> (tag: v1.2.3)
  Author: <git_author>
  Date:   Wed Nov 26 17:07:05 2025 +0000
  ...
  Notes:
      Note added after the fact, with additional detail.
  % git show v1.2.3
  tag v1.2.3
  Tagger: <git_author>
  Date:   Thu Nov 27 09:54:05 2025 +0000
...
** Investigate SSH Key used in =git clone=:
When running with: [[*Global git config to use per-VCS SSH keys for HTTPS/Git URLS:][Global git config to use per-VCS SSH keys for
HTTPS/Git URLS]], you can search the output of the following command to
figure out which SSH key is used:
...
- [[https://www.eircode.ie/benefits/overview][EIR Code - Irelands modern postcode system]].
*** API's:
...
** Trunk development + high trust team processes/expectations:
Key points around trunk development:

- *Pros:*
  - *Lead Time* = =Push commit -> Deployed= (Instead of: Push commit, wait for PR review, Deployed, Released).
  - *High Integration* = All dev's get changes more rapidly than long-lived branches.
  - *Highly Reactive* = Process to pull back from a Released breakage: =git revert <sha>= , push, CI time.
  - *High Trust culture* = The barrier to deploy changes is lower (both pushing good/broken code). Developers Own the Risk of making changes.
  - CI and Kubernetes probes are still doing the same job to prevent known breakages being Released to Customers
  - *Reduction in CI costs* = Only building =main= + pushed branches, instead of: =main= + PR's.
- *Cons:*
  - *Bus Count / Discoverability of changes*: Need post-merge Demo's, pre-merge buddying, Standup Comm's, etc to get Bus Count > 1.
  - *High churn can affect stability*. - People typically counter-act with long-lived branches + move back to PR flow. Instead of embracing constant change.
  - Intent of un-signposted concise changes can cause more conflicts for Devs overlapping in the same area.
  - *Requires diligence to use Feature Toggles* to not expose broken/partially-implemented changes.
- *Workflow differences:*
  - Always expect changes (get comfortable with merging/rebasing changes).
  - Down Tools on broken =main= - Culture change.
  - Git hooks are more important to enforce Quality standards / Definition of Done.
  - Concise commits, little & often (eg. The extreme view is to aim for 100 commits a day vs. 1).
    - Async Communication: Requires sign-posting intent/direction in code, especially if devs are overlapping in the same area.
  - Post-merge Demos of Deployed/Released functionality.
  - Feature Toggles!!
    - Every change should be behind a feature toggle to separate Deployed from Released.
    - Support gradual rollout of changes.
    - Rollback = toggle feature instead of =git revert=.
    - Clean up cost to remove feature toggle when no longer needed.

I've tried different styles, and currently swinging back to favouring
trunk development. I think it's a clear winner for anywhere that is
making a push on Lead Times that are measured from Push to Released,
since it removes the Human Review lag. I would suggest it as something
for repo maintainers to have as an option, but can keep fly-by-night
contributors on PR's only if you have Trust concerns.
